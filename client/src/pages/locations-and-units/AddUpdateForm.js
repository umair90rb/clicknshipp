...
import { Box, Modal, Button, ListItemText, MenuItem, Select, FormHelperText, Grid, InputLabel, OutlinedInput, Stack } from '@mui/material';
...
export default function AddUpdateForm({ type = '', data, visible, onClose }) {
  ...
  if (type === 'Location') {
    ...
    initialValues.type = data?.type || 'finished_products';
    validationSchema.address = Yup.string().max(255).required('Address is required');
    validationSchema.type = Yup.string().max(255).required('Type is required');
  }
              ...
              <Grid container spacing={3}>
                ...
                {type === 'Location' && (
                  <>
                    ...
                    <Grid item xs={12}>
                      <Stack spacing={1}>
                        <InputLabel htmlFor="type-signup">Store Type</InputLabel>
                        <Select
                          fullWidth
                          ...
                          labelId="type-signup"
                        >
                          {storeTypes.map(({ label, value }, index) => (
